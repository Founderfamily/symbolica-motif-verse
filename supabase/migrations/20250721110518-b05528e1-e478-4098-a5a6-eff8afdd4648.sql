-- Créer la fonction pour récupérer les symboles d'une collection avec les périodes temporelles
...
BEGIN
  ...
  SELECT 
    ...
    cp.name as cultural_period_name
  FROM public.collection_symbols cs
  JOIN public.symbols s ON cs.symbol_id = s.id
  LEFT JOIN public.cultural_periods cp ON s.culture = cp.culture AND s.period = cp.period
