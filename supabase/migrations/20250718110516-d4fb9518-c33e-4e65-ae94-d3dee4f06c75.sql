-- Final collection reorganization - simplified approach with proper position updates
...
-- Step 6: Reorder positions in all collections using a simpler approach
-- Create temp table with new positions
CREATE TEMP TABLE new_positions AS
SELECT 
  collection_id,
  symbol_id,
  row_number() OVER (PARTITION BY collection_id ORDER BY symbol_id) as new_position
...
-- Update positions using the temp table
UPDATE collection_symbols 
SET position = np.new_position
FROM new_positions np
WHERE collection_symbols.collection_id = np.collection_id 
AND collection_symbols.symbol_id = np.symbol_id;

-- Drop the temporary tables
DROP TABLE symbol_reassignments;
DROP TABLE new_positions;